...
<script type="text/javascript">
	...
	function classifyTitle(id) {
		$.ajax({
			url : '${basePath }classify/getParent.json',
			data : {
				'id' : id
			},
			type : 'get',
	...
	function classifyDtl(id) {
		$.ajax({
			url : '${basePath }classify/getChildClassifyAndBlogById',
			data : {
				'id' : id
			},
			type : 'post',
	...
	function renameClassify(classifyName, classifyId) {
		$.ajax({
			url : '${basePath }classify/' + classifyId + '.json',
			data : {
				'classifyName' : classifyName,
			},
			type : "post",
	...
	$(document).ready(function() {
		...
		$("#deleteClassify").on("click",function() {
			...
			$.get('${basePath }classify/'+ classifyId+ '.json',
			function(data) {
				...
				<div class="classifyList">
					...
					<div class="title">
						<div id="classifyLocation" style="float: left">
							位置:<span>/</span><input class="createInput" type="text" /> 
							<!-- 级别: --><span style="display: none">0</span>
							<!-- 文章数: --><span style="display: none">0</span> 
							<span style="display: none">0</span>
						</div>
						<div id="tip"
							style="float: left; display: none; color: red; -webkit-animation: bounce-up 1.4s linear infinite;">
							<span></span>
						</div>
						...
						<div id="classifyEnter"
							...
							<span><a href="javascript:void(0)" id="enter" data-type="">确认</a></span>
							<span><a href="javascript:void(0)" id="cancel">取消</a></span>
						</div>
					</div>
					<div class="clear"></div>
